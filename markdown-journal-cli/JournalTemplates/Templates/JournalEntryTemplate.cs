...
public class JournalEntryTemplate : ITemplateGenerator
{
    ...
    {
        var title = (
            parameters?.GetValueOrDefault("title", "Title goes here") ?? "Title goes here"
        ).ToString();
        var body = (
            parameters?.GetValueOrDefault("body", "body goes here.") ?? "body goes here."
        ).ToString();
...
";
    ...
    private static bool IsFalseString(string s)
    {
        ...
        return Array.Exists(
            falseStrings,
            fs => string.Equals(s.Trim(), fs, StringComparison.OrdinalIgnoreCase)
        );
    }
}
