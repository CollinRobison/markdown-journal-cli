...
public class AddTableOfContentsRollbackTests : IDisposable
{
    ...
    public AddTableOfContentsRollbackTests()
    {
        ...
        _journalConfiguration = new JournalConfiguration(_fileSystem, settingsOptions, NullLogger<JournalConfiguration>.Instance, fileTracking);
        _tableOfContentsService = new TableOfContentsService(_fileSystem, _journalConfiguration, settingsOptions, NullLogger<TableOfContentsService>.Instance);
        ...
        _fileSystem.CreateFile(JournalPath, ".journalrc", """{"journalName":"Test","tableOfContents":{"file":"1a-TableOfContents.md","extensions":[".md"],"structure":{"topics":[]},"rootEntries":[]}}""");
    ...
    public void Should_Delete_Created_Toc_When_FileWrite_Fails()
    {
        ...
        _fileSystem.InjectFaultOn(FaultInjectPoint.UpdateFile, 1, new IOException("TOC write failed"));
    ...
    public void Should_Restore_Journalrc_When_Toc_Creation_Fails_After_Config_Update()
    {
        ...
        var altJournalrcContent = """{"journalName":"Test","tableOfContents":{"file":"old-toc.md","extensions":[".md"],"structure":{"topics":[]},"rootEntries":[]}}""";
        ...
        _fileSystem.InjectFaultOn(FaultInjectPoint.UpdateFile, 2, new IOException("TOC write failed"));

        var command = CreateCommand();
        var result = command.Execute(null!, new AddTableOfContentsSettings
        {
